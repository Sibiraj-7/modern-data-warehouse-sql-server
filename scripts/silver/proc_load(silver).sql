-- Execute this procedure with "EXEC silver.load_silver"
...
BEGIN 
	...
	BEGIN TRY
		...
		SELECT 
			...
			END AS sls_due_dt,
			sls_quantity * ABS(COALESCE(sls_price,sls_sales / sls_quantity)) AS sls_sales,
			sls_quantity,
			ABS(COALESCE(sls_price,sls_sales / NULLIF(sls_quantity,0))) AS sls_price
